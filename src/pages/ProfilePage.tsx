...
const ProfilePage = () => {
  ...
  return (
    <section className="mx-auto flex w-full max-w-3xl flex-col gap-8">
      ...
      {/* User ID Section */}
      <div className="animate-stagger-6 card group hover:shadow-md transition-all duration-300">
        <div className="flex items-center gap-3 mb-4">
          <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-slate-200 group-hover:bg-slate-300 transition-colors">
        ...
        <div className="flex items-center justify-between gap-4 rounded-lg bg-slate-100 p-4">
          <p className="font-mono text-sm text-slate-700 break-all">{user.userId}</p>
          <button
            onClick={() => {
              navigator.clipboard.writeText(user.userId)
              alert('User ID copied to clipboard!')
            }}
            className="whitespace-nowrap rounded-lg bg-primary/10 px-3 py-2 text-xs font-semibold text-primary hover:bg-primary/20 transition-colors"
          >
            Copy
          </button>
        </div>
